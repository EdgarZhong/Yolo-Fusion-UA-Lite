...
RUN_NAME = "M5-FA-Concat-FPN-PAN-Rerun"
TOTAL_EPOCHS = 160
...
def transfer_single_backbone_to_dual(source_weights: Path, target_model) -> dict:
    """
    将单路 yolov8n 主干参数显式迁移到双路主干 (RGB/IR)
    遵循当前注意力实验的预训练权重加载规范
    """
...
class M5RerunOBBTrainer(OBBTrainer):
...
def main():
    ...
    trainer = M5RerunOBBTrainer(overrides=overrides)
    trainer.train()
